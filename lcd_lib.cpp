...
void setup() {
  ...
  custom_char_lcd(1, smiley);
  //comand_lcd(0x80); // Move cursor to first line
  shift_right_lcd(bals, 7, 1000);
}
...
void shift_left_lcd(uint8_t* arr,uint8_t len,long t){
  if(len>40){
    Serial.println("len is biger then the line buffer");
    return;;
  }
  arr_dat(arr, len);
  for(int i=0;i<len;i++){
...
void shift_right_lcd(uint8_t* arr,uint8_t len,long t){
  if(len>40){
    Serial.println("len is biger then the line buffer");
    return;;
  }
  ...
  arr_dat(arr, len);
  uint8_t tosh=max(16,len);

  for(int i=0;i<tosh;i++){
    delay(t);
    comand_lcd(0x1C);
  }
}

void custom_char_lcd(byte pos,byte img[]){
  pos &=0x07;

  comand_lcd(0x40|(pos << 3));
