...
#define EEPROM_ADDRESS 0x55  // Base address is 0x50, A0=1 adds 1
...
uint8_t boot_blocks = ceil((double)(n_bloks+2)/(double)(block_size));
...
void init_fat(){
  ...
  uint16_t len = min(n_bloks, read_len);
  readSequential(2, fat, n_bloks);
}
